...
С VLESS через веб-сокеты по какой-то причине не работает клиент Hiddify. Вместо него используйте другие клиенты, например v2RayTun, Happ, Streisand и другие.

## Уменьшение вероятности падения туннеля

Рекомендуется на уровне роутинга запретить доступ к доменам *geosite:category-speedtest*, поскольку любой тест скорости рушит туннель. Повышенная нагрузка на туннель так же плохо влияет на его работоспособность. Надзорный скрипт, впрочем, в случае обрушения перезагрузит его автоматически. 
...
Затем заново запустите ```vk-tunnel```, выполните вход путём открытия авторизационной ссылки в браузере и нажмите Enter. На один аккаунт VK можно авторизовать только один туннель.
